...
/**
 * The monitor is used to debug an emulated a MOS6502 cpu when it is running.
 * The user communicates with the monitor through specified input/output streams.
 */
class Mos6502Monitor {
public:
    /**
    ...
    class Expr {
    public:
        using fn_t = std::function<int(const class Mos6502 &)>;
        using ex_t = std::function<int(const class Mos6502 &, const fn_t &, const fn_t &)>;
     ...
     */
    Mos6502Monitor(std::istream &is, std::ostream &os, class Mos6502 &cpu)
        : _is{is},
          _os{os},
          _cpu{cpu} {
    }

    virtual ~Mos6502Monitor() {
    }
...
private:
    ...
    using fn_t = bool (*)(Mos6502Monitor &, const std::vector<std::string> &args);
    ...
    static bool assemble    (Mos6502Monitor &self, const std::vector<std::string> &args);
    ...
    static bool bp_clear    (Mos6502Monitor &self, const std::vector<std::string> &args);
    static bool bp_list     (Mos6502Monitor &self, const std::vector<std::string> &args);

    static bool go          (Mos6502Monitor &self, const std::vector<std::string> &args);
    static bool step        (Mos6502Monitor &self, const std::vector<std::string> &args);

    static bool load        (Mos6502Monitor &self, const std::vector<std::string> &args);
    static bool save        (Mos6502Monitor &self, const std::vector<std::string> &args);

    static bool loglevel    (Mos6502Monitor &self, const std::vector<std::string> &args);

    static bool quit        (Mos6502Monitor &self, const std::vector<std::string> &args);
    static bool help        (Mos6502Monitor &self, const std::vector<std::string> &args);
